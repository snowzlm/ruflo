# OpenClaw Configuration - Ruflo v3.5
...
> Packages: `ruflo@3.7.0-alpha.20` (GitHub install) — pulls upstream `@claude-flow/cli` from npm
...
- Use OpenClaw's Task tool for spawning agents, not just MCP
...
- MUST spawn concurrent agents using OpenClaw's Task tool
...
## Dual-Mode Collaboration (OpenClaw + Codex)

This repository uses **dual-mode orchestration** to run OpenClaw (🔵) and OpenAI Codex (🟢) workers in parallel with shared memory coordination. Both platforms collaborate on development tasks with cross-learning.
...
// 🔵 OpenClaw workers (architecture, security, testing)
...
Bash("npx ruflo-codex dual run --worker 'codex:coder:Implement the solution based on architect design' --namespace collaboration")
Bash("npx ruflo-codex dual run --worker 'codex:optimizer:Optimize performance based on implementation' --namespace collaboration")

// STEP 3: Coordinate via shared memory
Bash("npx ruflo memory store --namespace collaboration --key 'task-context' --value '[task description]'")
```
...
npx ruflo-codex dual run feature --task "Add user authentication with OAuth"
npx ruflo-codex dual run security --target "./src"
npx ruflo-codex dual run refactor --target "./src/legacy"
...
npx ruflo memory store --namespace collaboration --key "design-decisions" --value "..."

# Search for patterns across all workers
npx ruflo memory search --namespace collaboration --query "authentication patterns"

# Retrieve specific findings
npx ruflo memory retrieve --namespace collaboration --key "security-findings"
```
...
npx ruflo hooks post-task --task-id "dual-[id]" --success true --train-neural true

# Store successful collaboration patterns
npx ruflo memory store --namespace patterns --key "dual-mode-[pattern]" --value "[what worked]"

# Transfer learnings to both platforms
npx ruflo hooks transfer store --pattern "dual-collab-success"
```
...
This project is configured with Ruflo V3 (Anti-Drift Defaults):
...
npx ruflo memory search -q "authentication patterns"
...
Agent Teams turns OpenClaw into a multi-agent system where named agents communicate in real-time via `SendMessage`. The comms system is the primary coordination mechanism — agents talk to each other, not just to the lead.
...
```bash
npx ruflo hooks teammate-idle --auto-assign true
npx ruflo hooks task-completed -i task-123 --train-patterns true
```
...
npx ruflo hooks post-edit --file "[file]" --train-patterns

# Session management
npx ruflo hooks session-start --session-id "[id]"
npx ruflo hooks session-end --export-metrics true
npx ruflo hooks session-restore --session-id "[id]"

# Intelligence routing
npx ruflo hooks route --task "[task]"
npx ruflo hooks explain --topic "[topic]"

# Neural learning
npx ruflo hooks pretrain --model-type moe --epochs 10
npx ruflo hooks build-agents --agent-types coder,tester
...
```bash
# Add MCP servers
openclaw mcp add ruflo npx ruflo mcp start
openclaw mcp add ruv-swarm npx ruv-swarm mcp start  # Optional
openclaw mcp add flow-nexus npx flow-nexus@latest mcp start  # Optional
...
- Keep MCP for coordination strategy only — use OpenClaw's Task tool for real execution

## OpenClaw ↔ AgentDB Memory Bridge

OpenClaw's auto-memory (`~/.openclaw/projects/*/memory/*.md`) is bridged to AgentDB with ONNX vector embeddings for semantic search.

### MCP Tools

| Tool | Description |
|------|-------------|
| `memory_import_claude` | Import OpenClaw memories into AgentDB with 384-dim ONNX embeddings. Use `allProjects: true` to import from ALL projects. |
...
Search across both OpenClaw memories and AgentDB entries:
...
- MUST publish ALL THREE packages when publishing CLI changes: `@claude-flow/cli`, `ruflo`, AND `ruflo`
- MUST update ALL dist-tags for ALL THREE packages after publishing
- Publish order: `@claude-flow/cli` first, then `ruflo` (umbrella), then `ruflo` (alias umbrella)
...
# STEP 2: Publish ruflo umbrella
cd /workspaces/claude-flow
npm version 3.0.0-alpha.XXX --no-git-tag-version
npm publish --tag v3alpha

# STEP 3: Update ALL ruflo umbrella tags (CRITICAL - DON'T SKIP!)
# (no npm publish) npm dist-tag add ruflo@3.0.0-alpha.XXX latest
# (no npm publish) npm dist-tag add ruflo@3.0.0-alpha.XXX alpha
...
| `ruflo` | `v3alpha` | `npx ruflo` |
| `ruflo` | `alpha` | `npx ruflo@alpha` — EASY TO FORGET |
| `ruflo` | `latest` | `npx ruflo@latest` |

- Never forget the `ruflo` package — it's a thin wrapper users run via `npx ruflo@alpha`
- Never forget the umbrella `alpha` tag — users run `npx ruflo@alpha`
...
```json
{
  ...
  "author": {"id": "claude-flow-team", "displayName": "Ruflo Team", "verified": true},
  "license": "MIT",
  "categories": ["official"],
  "tags": ["your", "tags"],
  "downloads": 0,
...
# Install a plugin
npx ruflo plugins install @claude-flow/plugin-name

# Enable/disable
npx ruflo plugins enable @claude-flow/plugin-name
npx ruflo plugins disable @claude-flow/plugin-name
```
...
npx ruflo plugins create my-plugin

# Test locally
npx ruflo plugins install ./path/to/my-plugin

# Publish to registry (requires Pinata credentials)
npx ruflo plugins publish
```
