...
```bash
npx ruflo swarm init --topology mesh --max-agents 5 --strategy balanced
```

### Hierarchical for development

```bash
npx ruflo swarm init --topology hierarchical --max-agents 10 --strategy parallel --auto-spawn
```

### GitHub-focused swarm

```bash
npx ruflo swarm init --topology star --github --memory
```
...
Once initialized, use MCP tools in OpenClaw:
