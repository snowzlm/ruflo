...
description: Automated coordination, formatting, and learning from OpenClaw operations using intelligent hooks with MCP integration. Includes pre/post task hooks, session management, Git integration, memory coordination, and neural pattern training for enhanced development workflows.
---

# Hooks Automation

Intelligent automation system that coordinates, validates, and learns from OpenClaw operations through hooks integrated with MCP tools and neural pattern training.
...
- `.openclaw/settings.json` with hook configurations
...
npx ruflo init --hooks
```

This creates:
- `.openclaw/settings.json` with pre-configured hooks
- Hook command documentation in `.openclaw/commands/hooks/`
...
npx ruflo hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
npx ruflo hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
npx ruflo hook session-end --session-id "dev-session" --export-metrics
```
...
Examples:
  ...
  npx ruflo hook pre-edit -f "config/db.js" --validate-syntax
...
```bash
npx ruflo hook pre-bash --command <cmd>
...
Examples:
  npx ruflo hook pre-bash -c "rm -rf /tmp/cache"
  npx ruflo hook pre-bash --command "docker build ." --estimate-resources
```
...
```bash
npx ruflo hook pre-task [options]
...
Examples:
  npx ruflo hook pre-task --description "Implement user authentication"
  npx ruflo hook pre-task -d "Continue API dev" --load-memory
  npx ruflo hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
npx ruflo hook pre-search --query <query>
...
Examples:
  npx ruflo hook pre-search -q "authentication middleware"
```
...
Examples:
  ...
  npx ruflo hook post-edit -f "api/auth.js" --memory-key "auth/login"
  npx ruflo hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
npx ruflo hook post-bash --command <cmd>
...
Examples:
  npx ruflo hook post-bash -c "npm test" --update-metrics
```
...
```bash
npx ruflo hook post-task [options]
...
Examples:
  npx ruflo hook post-task --task-id "auth-implementation"
  npx ruflo hook post-task -t "api-refactor" --analyze-performance
  npx ruflo hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
npx ruflo hook post-search --query <query> --results <path>
...
Examples:
  npx ruflo hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
npx ruflo hook mcp-initialized --swarm-id <id>
...
```bash
npx ruflo hook agent-spawned --agent-id <id> --type <type>
...
```bash
npx ruflo hook task-orchestrated --task-id <id>
...
```bash
npx ruflo hook neural-trained --pattern <name>
...
```bash
npx ruflo hook memory-sync --namespace <ns>
...
```bash
npx ruflo hook session-start --session-id <id>
...
```bash
npx ruflo hook session-restore --session-id <id>
...
Examples:
  npx ruflo hook session-restore --session-id "swarm-20241019"
  npx ruflo hook session-restore -s "feature-auth" --restore-memory
```
...
Examples:
  ...
  npx ruflo hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
npx ruflo hook notify --message <msg>
...
Examples:
  npx ruflo hook notify -m "Task completed" --level info
  npx ruflo hook notify -m "Critical error" --level error --broadcast
```
...
Edit `.openclaw/settings.json` to configure hooks:
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx ruflo hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx ruflo hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx ruflo hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx ruflo hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx ruflo hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
npx ruflo hook pre-task --description "Build REST API"
...
```javascript
// Hook command
npx ruflo hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
npx ruflo hook session-end --session-id "dev-2024"
...
for FILE in $FILES; do
  ...
  npx ruflo hook pre-edit --file "$FILE" --validate-syntax
  ...
  npx ruflo hook post-edit --file "$FILE" --auto-format
done
...
# Run quality checks
npx ruflo hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(npx ruflo metrics score --format json | jq -r '.truth_score')
...
npx ruflo hook pre-task \
  --description "Implement user authentication API" \
  --auto-spawn-agents \
  --load-memory

# STEP 2: Work begins - pre-edit validation
npx ruflo hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax

# STEP 3: Edit file (via OpenClaw Edit tool)
# ... code changes ...

# STEP 4: Post-edit processing
npx ruflo hook post-edit \
  --file "api/auth.js" \
...
npx ruflo hook session-restore \
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "script",
            "script": ".openclaw/hooks/custom-quality-check.js"
          }
        ]
      }
    ]
...
npx ruflo hook session-start --session-id "fullstack-feature"
...
# Backend work
npx ruflo hook pre-edit --file "api/profile.js"
# ... implement backend ...
npx ruflo hook post-edit \
  --file "api/profile.js" \
...
npx ruflo hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
npx ruflo hook post-edit \
...
npx ruflo hook session-start --session-id "debug-memory-leak"
...
npx ruflo hook pre-search --query "EventEmitter"
# ... search executes ...
npx ruflo hook post-search \
  --query "EventEmitter" \
  --cache-results
...
npx ruflo hook pre-task --description "Analyze code complexity"
# ... analysis ...
npx ruflo hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  npx ruflo hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  npx ruflo hook post-edit \
    --file "$file" \
...
npx ruflo hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .claude-flow/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
npx ruflo hook validate-config
```
...
#### Hooks Not Executing
- Verify `.openclaw/settings.json` syntax
...
- `npx ruflo init --hooks` - Initialize hooks system
- `npx ruflo hook --list` - List available hooks
- `npx ruflo hook --test <hook>` - Test specific hook
- `npx ruflo memory usage` - Manage memory
- `npx ruflo agent spawn` - Spawn agents
- `npx ruflo swarm init` - Initialize swarm
