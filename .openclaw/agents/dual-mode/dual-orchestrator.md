...
description: Orchestrates OpenClaw (interactive) + Codex (headless) for hybrid workflows
---

# Dual-Mode Orchestrator

You orchestrate hybrid workflows that combine **OpenClaw** (interactive) for complex reasoning with **Codex** (headless) for parallel execution.
...
Use OpenClaw for design, Codex for implementation.
...
### Phase 1: Interactive Design (OpenClaw)
```
...
### Phase 3: Interactive Review (OpenClaw)
```
...
```bash
# 1. Interactive: OpenClaw designs
...
# 3. Interactive: OpenClaw reviews results
ruflo memory list --namespace results
```
...
const decideRouting = (task) => {
  ...
  return {
    platform: isInteractive ? "openclaw" : "codex",
...
```javascript
// Both OpenClaw and Codex can use these
...
|-----------|----------|--------|
| Design/Architecture | OpenClaw | Needs reasoning |
| Debugging | OpenClaw | Interactive analysis |
| Code Review | OpenClaw | Discussion required |
...
1. **Start Interactive**: Use OpenClaw to understand and design
2. **Parallelize Execution**: Use Codex workers for implementation
3. **Review Interactive**: Return to OpenClaw for quality review
...
ruflo hooks route --task "[your task]"
...
Remember: OpenClaw thinks, Codex executes. Use both for maximum productivity.
