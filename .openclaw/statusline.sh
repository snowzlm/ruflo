...
# Read OpenClaw JSON input from stdin (if available)
CLAUDE_INPUT=$(cat 2>/dev/null || echo "{}")

# Get project directory from OpenClaw input or use current directory
...
SWARM_COMMS="${PROJECT_DIR}/.openclaw/helpers/swarm-comms.sh"
QUEUE_PENDING=0
...
# Get context window usage from Context Autopilot state (primary) or OpenClaw input (fallback)
CONTEXT_PCT=0
CONTEXT_TOKENS=""
CONTEXT_COLOR="${DIM}"
AUTOPILOT_STATUS=""
...
if [ -f "$AUTOPILOT_STATE" ]; then
  ...
  # Fallback: read from OpenClaw input JSON
...
# Get model name from OpenClaw input
MODEL_NAME=""
