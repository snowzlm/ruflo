...
// Read stdin — OpenClaw sends hook data as JSON via stdin
// Uses a timeout to prevent hanging when stdin is in an ambiguous state
// (not TTY, not a proper pipe) which happens with OpenClaw hook invocations.
...
main().catch(function(e) {
  ...
  // Ensure clean exit for OpenClaw hooks
  process.exit(0);
});
