#!/usr/bin/env node
/**
 * Patch: Aggressive Text Pruning for OpenClaw
 *
 * Extends OpenClaw's micro-compaction (Vd function) to also prune old
...
function apply() {
  ...
  if (!src.includes(VD_CALL_PATTERN)) {
    ...
    console.error('OpenClaw may have been updated. Pattern expected:');
  ...
  console.log('');
  console.log('Restart OpenClaw for the patch to take effect.');
}
