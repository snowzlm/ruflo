...
`ruflo doctor` checks: Node 20+, npm 9+, git, config, daemon, memory DB, API keys, MCP servers, disk space, TypeScript.

## Hooks Quick Reference

```bash
ruflo hooks pre-task --description "[task]"
ruflo hooks post-task --task-id "[id]" --success true
ruflo hooks session-start --session-id "[id]"
ruflo hooks route --task "[task]"
