...
ruflo github issue-to-swarm 456 \
...
ruflo github issue-analyze 456 \
  --suggest-agents \
  --estimate-complexity \
  --create-subtasks
```
...
SUBTASKS=$(ruflo github issue-decompose 456 \
  --body "$ISSUE_BODY" \
  --max-subtasks 10 \
  --assign-priorities)
...
# Get swarm progress
PROGRESS=$(ruflo github issue-progress 456)

# Update checklist in issue body
UPDATED_BODY=$(echo "$CURRENT" | jq -r '.body' | \
  ruflo github update-checklist --progress "$PROGRESS")
        ...
        with:
          command: |
            if [[ "${{ github.event.label.name }}" == "swarm-ready" ]]; then
              ruflo github issue-init ${{ github.event.issue.number }}
            fi
```
...
ruflo github feature-swarm 456 \
  --design \
  --implement \
  --document \
  --demo
...
echo "$STALE_ISSUES" | jq -r '.number' | while read -r num; do
  ...
  # Analyze with swarm
  ACTION=$(ruflo github analyze-stale \
    --issue "$ISSUE" \
    --suggest-action)
