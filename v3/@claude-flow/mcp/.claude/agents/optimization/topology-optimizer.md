...
npx ruflo topology-analyze --swarm-id <id> --metrics performance

# Optimize topology automatically
npx ruflo topology-optimize --swarm-id <id> --strategy adaptive

# Compare topology configurations
npx ruflo topology-compare --topologies ["hierarchical", "mesh", "hybrid"]

# Generate topology recommendations
npx ruflo topology-recommend --workload-profile <file> --constraints <file>

# Monitor topology performance
npx ruflo topology-monitor --swarm-id <id> --interval 60
```

### Agent Placement Commands
```bash
# Optimize agent placement
npx ruflo placement-optimize --algorithm genetic --agents <agent-list>

# Analyze placement efficiency
npx ruflo placement-analyze --current-placement <config>

# Generate placement recommendations
npx ruflo placement-recommend --communication-patterns <file>
```
