 ...
 *
 * Updated to support --claude flag for launching interactive OpenClaw sessions
...
async function spawnClaudeCodeInstance(swarmId, swarmName, objective, workers, flags) {
    output.writeln();
    output.writeln(output.bold('🚀 Launching OpenClaw with Hive Mind Coordination'));
    ...
    try {
        ...
        try {
            ...
            output.writeln();
            output.printWarning('OpenClaw CLI not found in PATH');
            output.writeln(output.dim('Install it with: npm install -g @anthropic-ai/openclaw'));
        ...
        if (claudeAvailable && !dryRun) {
            ...
            //   3. ~/.claude.json or ~/.openclaw/mcp.json (user-global)
            ...
            if (mcpConfigPath) {
                // #1780 — OpenClaw's `--mcp-config` is variadic; passing it as two
            ...
            output.writeln();
            output.printInfo('Launching OpenClaw...');
            ...
            const sigintHandler = () => {
                ...
                output.writeln();
                output.writeln();
                output.printWarning('Pausing session and terminating OpenClaw...');
            ...
            claudeProcess.on('exit', (code) => {
                ...
                if (code === 0) {
                    output.writeln();
                    output.printSuccess('OpenClaw completed successfully');
                }
                else if (code !== null) {
                    output.writeln();
                    output.printError(`OpenClaw exited with code ${code}`);
                }
            });
            output.writeln();
            output.printSuccess('OpenClaw launched with Hive Mind coordination');
            ...
            output.writeln();
            output.printInfo('Dry run - would execute OpenClaw with prompt:');
            ...
            output.printList([
                'Install OpenClaw: npm install -g @anthropic-ai/openclaw',
        ...
        try {
            ...
            output.writeln(output.dim('You can run OpenClaw manually with the saved prompt'));
...
const initCommand = {
    ...
    action: async (ctx) => {
        ...
        try {
            ...
            output.writeln(output.dim('  Use "claude-flow hive-mind spawn --claude" to launch OpenClaw'));
            return { success: true, data: result };
        }
        catch (error) {
...
const spawnCommand = {
    name: 'spawn',
    description: 'Spawn worker agents into the hive (use --claude to launch OpenClaw)',
    options: [
        {
            name: 'count',
            short: 'n',
        ...
        // NEW: --claude flag for launching OpenClaw
        {
            name: 'claude',
            description: 'Launch OpenClaw with hive-mind coordination prompt',
            type: 'boolean',
            default: false
        },
        {
        ...
        },
        {
            ...
            description: 'Skip permission prompts in OpenClaw (use with caution)',
            type: 'boolean',
            default: true
        },
        {
        ...
        {
            name: 'dry-run',
            description: 'Show what would be done without launching OpenClaw',
            type: 'boolean',
            default: false
        },
        {
            name: 'non-interactive',
            description: 'Run OpenClaw in non-interactive mode',
            type: 'boolean',
            default: false
        },
        {
    ...
    examples: [
        ...
        { command: 'claude-flow hive-mind spawn --claude -o "Build a REST API"', description: 'Launch OpenClaw with objective' },
        { command: 'claude-flow hive-mind spawn -n 5 --claude -o "Research AI patterns"', description: 'Spawn workers and launch OpenClaw' }
    ],
    action: async (ctx) => {
        ...
        try {
            ...
            if (launchClaude) {
                ...
                // Launch OpenClaw with hive mind prompt
...
export const hiveMindCommand = {
    ...
    examples: [
        ...
        { command: 'claude-flow hive-mind spawn --claude -o "Build a feature"', description: 'Launch OpenClaw with hive mind' },
    ...
    action: async () => {
        ...
        output.printList([
            ...
            `${output.highlight('spawn')}           - Spawn worker agents (use --claude to launch OpenClaw)`,
        ...
        output.printList([
            ...
            output.success('NEW: --claude flag to launch interactive OpenClaw sessions')
        ]);
        output.writeln();
        output.writeln('Quick Start with OpenClaw:');
