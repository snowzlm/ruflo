...
hooks:
  pre: |
    ruflo hooks pre-task --description "Code analysis agent starting: ${description}" --auto-spawn-agents false
  post: |
    ruflo hooks post-task --task-id "analysis-${timestamp}" --analyze-performance true
metadata:
...
ruflo hooks pre-search --query "code quality metrics" --cache-results true

# Load project context
ruflo memory retrieve --key "project/architecture"
ruflo memory retrieve --key "project/standards"
```
...
ruflo memory store --key "analysis/code-quality" --value "${results}"

# Generate recommendations
ruflo hooks notify --message "Code analysis complete: ${summary}"
```
