...
```bash
npx ruflo plugins install --name @claude-flow/plugin-agentic-qe
```
...
```bash
ruflo mcp call aqe/generate-tests \
...
```bash
ruflo mcp call aqe/security-scan \
  --targetPath ./src \
  --scanType sast \
...
```bash
ruflo mcp call aqe/predict-defects \
...
ruflo mcp call aqe/chaos-inject \
