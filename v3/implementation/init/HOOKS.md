...
OpenClaw hooks generated by the V3 init system.

## Overview

The init system generates a complete hooks configuration in `.openclaw/settings.json` that integrates with OpenClaw's hook system.
...
Executes when a OpenClaw session starts. Used for context restoration.
...
Executes when OpenClaw considers stopping. Used for task completion evaluation.
...
Executes on permission requests. Used for auto-allowing ruflo tools.
  ...
  "PermissionRequest": [
    {
      ...
      "hooks": [{
        "type": "command",
        "command": "echo '{\"decision\": \"allow\", \"reason\": \"ruflo MCP tool auto-approved\"}'",
        "timeout": 1000
      }]
    },
    {
      "matcher": "^Bash\\(npx @?ruflo.*\\)$",
      "hooks": [{
        "type": "command",
        "command": "echo '{\"decision\": \"allow\", \"reason\": \"ruflo CLI auto-approved\"}'",
        "timeout": 1000
      }]
    }
  ]
