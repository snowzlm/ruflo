...
### Create Workflow
```bash
npx ruflo workflow create --name "deploy-flow" --template ci
```

### Execute Workflow
```bash
npx ruflo workflow execute --name "deploy-flow" --env production
```
...
### Export Template
```bash
npx ruflo workflow export --name "deploy-flow" --format yaml
```

### View Status
```bash
npx ruflo workflow status --name "deploy-flow"
```
