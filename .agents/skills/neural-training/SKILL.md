...
### Train Patterns
```bash
npx ruflo neural train --model-type moe --epochs 10
```
...
### View Patterns
```bash
npx ruflo neural patterns --type all
```

### Predict
```bash
npx ruflo neural predict --input "task description"
```

### Optimize
```bash
npx ruflo neural optimize --target latency
```
