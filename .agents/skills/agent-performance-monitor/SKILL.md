...
npx ruflo bottleneck-analyze --component swarm-coordination

# Health check all components
npx ruflo health-check --components ["swarm", "agents", "coordination"]

# Collect specific metrics
npx ruflo metrics-collect --components ["cpu", "memory", "network"]

# Monitor SLA compliance
npx ruflo sla-monitor --service swarm-coordination --threshold 99.9
```

### Alert Configuration
```bash
# Configure performance alerts
npx ruflo alert-config --metric cpu_usage --threshold 80 --severity warning

# Set up anomaly detection
npx ruflo anomaly-setup --models ["statistical", "ml", "time_series"]

# Configure notification channels
npx ruflo notification-config --channels ["slack", "email", "webhook"]
```
