...
ReasoningBank from agentic-flow v1.4.11 is now fully integrated into ruflo, providing a closed-loop memory system that improves agent task success rates from 23% to 98% through experience-based learning.
...
```bash
# Install ruflo with ReasoningBank support
npm install ruflo@latest

# Initialize memory system
npx ruflo agent memory init

# Verify installation
npx ruflo agent memory status
```
...
ruflo agent run coder "Build REST API with auth" --enable-memory

# Second execution (learns from first attempt)
ruflo agent run coder "Add JWT authentication" --enable-memory --memory-domain api

# Third execution (retrieves top 5 relevant memories)
ruflo agent run coder "Implement OAuth2 flow" --enable-memory --memory-k 5
```
...
ruflo agent memory list --domain api --limit 10
...
ruflo agent run coder "Build API" --enable-memory --provider anthropic

# OpenRouter (99% cost savings, still learns)
ruflo agent run coder "Add endpoints" --enable-memory --provider openrouter

# ONNX (free local, learns from local patterns)
ruflo agent run coder "Write tests" --enable-memory --provider onnx

# Gemini (free tier, learns efficiently)
ruflo agent run coder "Document code" --enable-memory --provider gemini
```
...
# 4. List memories
ruflo agent memory list --limit 10

# 5. Run agent with memory
ruflo agent run coder "Build calculator" --enable-memory --provider onnx

# 6. Verify memory was created
ruflo agent memory status  # Should show 1+ memories
```
...
```javascript
import { AgentExecutor } from 'ruflo';
...
ruflo agent run coder "task" --enable-memory --provider onnx
```
...
ruflo agent run coder "task" --enable-memory --memory-learning true
...
ruflo agent run coder "API task" --enable-memory --memory-domain api

# 2. Increase k for complex tasks
ruflo agent run coder "Complex feature" --enable-memory --memory-k 10

# 3. Consolidate regularly (dedup + prune)
ruflo agent memory consolidate

# 4. Use appropriate provider for task
ruflo agent run coder "Quick fix" --enable-memory --provider onnx  # Fast
ruflo agent run coder "Critical bug" --enable-memory --provider anthropic  # Best
```
...
3. **Start using** memory in your workflows:
   ```bash
   ruflo agent run coder "Your task" --enable-memory
   ```
...
- **Claude-Flow Docs**: https://github.com/snowzlm/ruflo
...
Users can now run `ruflo agent run coder "task" --enable-memory` to leverage experience-based learning that improves success rates from 23% to 98%.
