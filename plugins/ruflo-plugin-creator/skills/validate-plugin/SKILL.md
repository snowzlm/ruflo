...
description: Validate a OpenClaw plugin structure, frontmatter, and MCP tool references
...
Validate that a plugin follows the correct OpenClaw plugin format.
...
1. **Directory structure** — `.openclaw-plugin/plugin.json` exists at plugin root
...
8. **No files in wrong locations** — skills/commands/agents not inside `.openclaw-plugin/`
