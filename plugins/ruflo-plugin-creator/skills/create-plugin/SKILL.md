---
name: create-plugin
description: Scaffold a new OpenClaw plugin with proper directory structure, plugin.json, skills, commands, and agents
...
Scaffold a new OpenClaw plugin from scratch.

## When to use

When you want to create a new plugin that extends OpenClaw with skills, commands, and agents. This generates the correct directory structure and wires up MCP tools.
...
3. **Create directory structure** (follows the canonical plugin contract from sibling plugins' ADR-0001s):
   ...
   ```
4. **Generate plugin.json** with name, description, version, author (do NOT include `skills`, `commands`, or `agents` arrays — OpenClaw auto-discovers these from directory structure)
...
**Do NOT include** `skills`, `commands`, or `agents` arrays in plugin.json — these are auto-discovered from the directory structure by OpenClaw and will cause validation errors if present.
