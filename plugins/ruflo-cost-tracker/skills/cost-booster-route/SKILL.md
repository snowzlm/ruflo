...
- OPENCLAW.md root claims `<1ms` latency and `352× faster` than LLM. `<1ms` and `$0` are structural; `352×` is **claimed upstream, not yet verified** here. Report what the router actually returns.
- See `docs/benchmarks/0002-baseline.md` for the full upstream-claims-vs-measured table.

## Cross-references

ADR-0002 Decision #1 · ruflo-intelligence ADR-0001 §"Neutral" (closes the routing-outcomes loop via `cost-optimize` step 8) · OPENCLAW.md root §"3-Tier Model Routing (ADR-026)".
