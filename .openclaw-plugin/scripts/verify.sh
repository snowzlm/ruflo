...
if [ -d "$HOME/.openclaw/commands" ]; then
    CMD_COUNT=$(find "$HOME/.openclaw/commands" -name "*.md" 2>/dev/null | wc -l | tr -d ' ')
...
if [ -d "$HOME/.openclaw/agents" ]; then
    AGENT_COUNT=$(find "$HOME/.openclaw/agents" -name "*.md" 2>/dev/null | wc -l | tr -d ' ')
...
# Check settings
info "Checking OpenClaw settings..."
if [ -f "$HOME/.openclaw/settings.json" ]; then
    success "Settings file exists"

    if grep -q "claude-flow" "$HOME/.openclaw/settings.json"; then
