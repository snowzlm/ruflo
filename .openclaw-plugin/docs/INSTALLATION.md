# 📦 Ruflo Plugin Installation Guide
...
# In OpenClaw, install the plugin
/plugin add .
```
...
Look for `ruflo` in the active plugins list.
...
- **ruflo**: Core orchestration (40+ tools) - Required
...
Look for `ruflo` in the list with status "active".

### Test Commands

Type `/` in OpenClaw and look for:
...
Agents are automatically available for OpenClaw to delegate to when appropriate.
