...
[![OpenClaw](https://img.shields.io/badge/OpenClaw-Plugin-D97757?style=for-the-badge&logoColor=white&logo=npm)](https://github.com/snowzlm/ruflo)
...
**Multi-agent AI orchestration for OpenClaw**

> **🌙 OpenClaw fork (snowzlm/ruflo)** — Multi-provider (Claude / GPT / Gemini / OpenClaw gateway), GitHub-only install (no npm publish), private deployment.
>
> ```bash
> # install from THIS GitHub repo (not npm)
> git clone https://github.com/snowzlm/ruflo && cd ruflo && npm install && npm link
> # or: npm install -g github:snowzlm/ruflo
> ```
> Set any of: `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GEMINI_API_KEY` / `OPENCLAW_GATEWAY_TOKEN` — Ruflo auto-detects available providers.



</div>

Orchestrate 100+ specialized AI agents across machines, teams, and trust boundaries. Ruflo adds coordinated swarms, self-learning memory, federated comms, and enterprise security to OpenClaw — so agents don't just run, they collaborate.
...
One `npx ruvflo init` gives OpenClaw a nervous system: agents self-organize into swarms, learn from every task, remember across sessions, and — with federation — securely talk to agents on other machines without leaking data. You keep writing code. Ruflo handles the coordination.
...
> **New to Ruflo?** You don't need to learn 314 MCP tools or 26 CLI commands. After `init`, just use OpenClaw normally -- the hooks system automatically routes tasks, learns from successful patterns, and coordinates agents in the background.
...
| | **OpenClaw Plugin** | **CLI install (`npx ruflo init`)** |
|---|---|---|
...
| Files in your workspace | **Zero** | `.openclaw/`, `.claude-flow/`, `OPENCLAW.md`, helpers, settings |
| MCP server registered | **No** (`memory_store`, `swarm_init`, etc. unavailable to Claude) | Yes |
| Hooks installed | No | Yes |
| Best for | Try a single plugin's commands without committing to the full install | Production use — everything works as documented |

### Path A — OpenClaw Plugins (lite, slash commands only)
...
```bash
# Add Ruflo as an MCP server in OpenClaw (canonical form, matches USERGUIDE.md)
...
| 🧩 **Plugin Marketplace** | 32 native OpenClaw plugins + 21 npm plugins |
...
ruflo federation send --to team-b --type task-request \
...
See [issue #1669](https://github.com/snowzlm/ruflo/issues/1669) for the complete architecture, trust model, and implementation roadmap.
...
# Or via CLI
ruflo plugins install @claude-flow/plugin-agent-federation
```

<details>
<summary><strong>OpenClaw: With vs Without Ruflo</strong></summary>

| Capability | OpenClaw Alone | + Ruflo |
|------------|-------------------|---------|
...
| Issues & Bugs | [GitHub Issues](https://github.com/snowzlm/ruflo/issues) |
